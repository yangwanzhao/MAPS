...
from sam2.modeling.maps_modules import AdaptiveHuFilterEMA
...
class SAM2VideoPredictor(SAM2Base):
    ...
    def _get_orig_video_res_output(self, inference_state, any_res_masks):
        ...
        WY: This makes the video_res_masks has dim [1, 1, 1080, 1920]
        """
        device = inference_state["device"]
        video_H = inference_state["video_height"]
        video_W = inference_state["video_width"]
        ...
        if any_res_masks.shape[-2:] == (video_H, video_W):  # [1, 1, 256, 256][-2:] == (1080, 1920)
    ...
    def propagate_in_video(
        ...
        #########################
        # MAPS modules: Maintain the Hu Moment value here
        hu_manager = None
        if self.is_maps:
            if run_name is None:
                hu_manager = AdaptiveHuFilterEMA()
            else:
                log_path = f'/home/wan/sam2/notebooks-MAPS/hu_logs/{run_name[0]}'
                os.makedirs(log_path, exist_ok=True)
                hu_manager = AdaptiveHuFilterEMA(os.path.join(log_path, f'{run_name[1]}.txt'))

        assert batch_size == 1, 'We only deal with one object.'
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            for obj_idx in range(batch_size):
                obj_output_dict = inference_state["output_dict_per_obj"][obj_idx]
                # WY: In my case where the prompt exists only in 1st frame, obj_output_dict["cond_frame_outputs"] always contains only frame 0.
                if frame_idx in obj_output_dict["cond_frame_outputs"]: 
                    ...
                    # MAPS: Always add the first frame's prediction
                    if self.is_maps and hu_manager is not None:
                        hu_manager.initialize(pred_masks)
                else:
                    ...
                    if self.is_maps:
                        valid_hu_frame = current_out['is_valid_hu']
            ...
            ####################### Visualize any_res_mask and video_res_masks
            # from PIL import Image
            # input_mask_np = all_pred_masks.squeeze().cpu()  # Remove batch and channel dimensions
            # output_mask_np = video_res_masks.squeeze().cpu()

            # input_mask_np = (input_mask_np > 0).float().numpy()
            # output_mask_np = (output_mask_np > 0).float().numpy()

            # # Save the masks as image files
            # input_mask_pil = Image.fromarray((input_mask_np * 255).astype("uint8"))  # Scale to 0-255 and convert to uint8
            # output_mask_pil = Image.fromarray((output_mask_np * 255).astype("uint8"))

            # input_mask_pil.save(f"pics/original_mask_{frame_idx}.png")
            # output_mask_pil.save(f"pics/interpolated_mask_{frame_idx}.png")
            ####################### End
            ####################### Conclusion: After comparing the low res and high res results, I decided to directly work on low res masks.

            if self.is_maps:
                yield frame_idx, obj_ids, video_res_masks, valid_hu_frame
            else:
    ...
    def _cal_Hu_Moments(self, mask):
        assert mask.size() == (1, 1, 256, 256), mask.size()
        return torch.rand(1)
    ...
    def _run_single_frame_inference(
        ...
        # MAPS
        is_valid_hu = current_out["is_valid_hu"]
