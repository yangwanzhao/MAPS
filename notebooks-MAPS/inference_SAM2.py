...
if __name__ == '__main__':
    ...
    NUM_PROMPTS = 1
    output_video = f'check_SAM2_tiny.mp4'
    output_txt = f'bboxes_SAM2_tiny.txt'
    output_mask = f'frames_SAM2_tiny'
    ...
    for id, inst_id in enumerate(os.listdir(DATA_ROOT)):
        ...
        if os.path.exists(mask_dir):
            ...
            if len(files) == 900:
                print(f'Inference SAM2 on {inst_id} [{id+1}/{len(os.listdir(DATA_ROOT))}] [SKIPPED]')
                continue
            else:
                shutil.rmtree(mask_dir)
                os.makedirs(mask_dir)
                print(f'Inference SAM2 on {inst_id} [{id+1}/{len(os.listdir(DATA_ROOT))}]')
        else:
            os.makedirs(mask_dir)
            print(f'Inference SAM2 on {inst_id} [{id+1}/{len(os.listdir(DATA_ROOT))}]')
        ...
        picked_prompts = pick_prompts_from_bbox_file(bbox_src, NUM_PROMPTS)
